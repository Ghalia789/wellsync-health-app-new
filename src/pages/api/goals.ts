...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === "DELETE") {
      ...
      // 2️⃣ Sauvegarder dans l’historique
      await GoalHistory.create({
        ...
        status: goal.status,
        sourceGoalId: goal._id.toString(),
      });

      // 3️⃣ Supprimer le goal normalement
      await Goal.findByIdAndDelete(goal._id);

      return res.status(200).json({ message: "Goal deleted & archived" });
    }
